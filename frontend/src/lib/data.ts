...
addFormats(ajv);
const resumeSchemaPath = path.resolve(
  process.cwd(),
  "src",
  "lib",
  "schema",
...
export function loadResume(strictSchema = false): {
  ...
  const resumePath = path.resolve(process.cwd(), "..", "resume", "resume.yaml");
  const text = fs.readFileSync(resumePath, "utf-8");
  const data = YAML.parse(text) as ResumeRoot;
  const errors: ValidationIssue[] = [];
  const ok = validateResume(data as unknown);
...
export function getSectionOrder(config: SiteConfigRoot): string[] {
  const explicit = config.sections?.order;
