...
def build_agent_context(resume: dict, site_config: dict) -> dict[str, Any]:
    """
...
def _build_contact(basics: dict) -> dict[str, Any]:
    return {
        "email": basics.get("email"),
        "url": basics.get("url"),
        "location": _format_location(basics.get("location", {})),
        "profiles": [
            {"network": p.get("network"), "url": p.get("url"), "username": p.get("username")}
...
def _build_work_entry(w: dict) -> dict[str, Any]:
    return {
        "company": w.get("name"),
...
def _build_education_entry(e: dict) -> dict[str, Any]:
    return {
...
def _group_skills(skills: list[dict]) -> dict[str, list[str]]:
...
def _build_project_entry(p: dict) -> dict[str, Any]:
    return {
        "name": p.get("name"),
...
def _build_certificate_entry(c: dict) -> dict[str, Any]:
    return {
        "name": c.get("name"),
        "issuer": c.get("issuer"),
        "date": str(c.get("date", "")),
