...
# Skip entire module if Docker daemon is not available
...
def whisper_container() -> Generator[DockerContainer]:
...
@pytest.fixture(scope="module")
def kokoro_container() -> Generator[DockerContainer]:
...
def test_whisper_server(whisper_container: DockerContainer) -> None:
...
def test_kokoro_server(kokoro_container: DockerContainer) -> None:
    ...
    # Even if it's 404 for this exact path, we verify the server is running
    # and responding to HTTP
    # Kokoro FastAPI has /v1/audio/speakers or similar? We'll just check it's
