...
│   ├── common/                # Shared helper libraries, base models, and config
│   ├── auth/                  # FastAPI LiveKit room token server
│   ├── resume/                # RenderCV-based resume builder & generation pipeline
│   ├── tailor/                # FastAPI + LangGraph resume tailoring pipeline
│   ├── agent/                 # LiveKit Multimodal Voice & Text Agent
│   └── mcp/                   # FastMCP portfolio tools server
├── e2e/                       # Playwright-based Python end-to-end test suite
└── infra/                     # Docker Compose configurations and deployment scripts
```
...
- **Technology Stack**: Astro 5, React 19, Framer Motion, Tailwind CSS v4.
- **Data-Driven Resume**: Reads single-source structured YAML data (`resume/resume.yaml` and `site-config.yaml`) and compiles them into the layout dynamically.
...
- **Technology Stack**: Python, RenderCV, Typst backend.
- **Function**: Takes `resume/resume.yaml`, validates it against the schema, and compiles it into:
  - High-fidelity PDF copy (`frontend/public/resume.pdf`)
  - Clean Markdown format (`frontend/public/resume.md`)
  - Raw JSON and YAML formats (`frontend/public/resume.json`, `frontend/public/resume.yaml`)
  - Structured agent context (`backend/agent/src/agent/portfolio_content.json`)
