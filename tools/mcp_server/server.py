...
def init_resume(resume_yaml: str) -> str:
    """Initializes the working resume context for the session.

    Call this before any editing tools.
    Args:
...
def append_item(path: str, new_object_json: str) -> str:
    ...
    Args:
        ...
        new_object_json: The new dictionary/string to append,
            provided as a valid JSON string.
    """
    global working_resume
    d = working_resume
    try:
...
def insert_item(path: str, index: int, new_object_json: str) -> str:
    ...
    Args:
        ...
        new_object_json: The new dictionary/string to insert,
            provided as a valid JSON string.
    """
    global working_resume
    d = working_resume
...
def format_resume(path: str) -> str:
    """Returns the current state of the working resume at the path in YAML format.

    Use path 'root' or '' for the whole resume.
    """
    global working_resume
    keys = path.split(".")
    d = working_resume
...
def evaluate_ats_score(job_description: str) -> str:
    """Evaluates the candidate's current working resume against the job description

    and returns an ATS score (0-100) with suggestions.
    Args:
        job_description: The job description text.
    Returns:
        A report containing an ATS score and improvement suggestions
    ...
    As an expert Applicant Tracking System (ATS), evaluate this resume
