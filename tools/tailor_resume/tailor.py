...
def run_agentic_tailor(jd_text, original_resume, inferred_company="target_company"):
    """Uses google-adk LlmAgent to tailor the resume via MCP tools."""
    ...
    # Assuming MCP server updates its internal state, we format and
    # fetch the final resume
    try:
        # Note: In a complete MCP implementation we'd grab the final
        # YAML payload from the server.
        # For this script we will log completion. The actual UI agent
        # loops it dynamically.
...
def main():
    ...
    parser.add_argument(
        "--company", type=str, default="UnknownCompany", help="Target company name"
    )
    parser.add_argument("--resume-file", type=str, default="../../resume/resume.yaml")
    ...
    resume_path = os.path.abspath(
        os.path.join(os.path.dirname(__file__), args.resume_file)
    )
    ...
    tailored_dir = os.path.abspath(
        os.path.join(os.path.dirname(__file__), "../../src/content/tailored")
    )
