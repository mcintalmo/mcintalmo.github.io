...
def pull_from_linkedin(output_file: str, csv_path: str = "Profile.csv"):
    """
    Parses an official LinkedIn Data Export CSV and writes the basic details
    to the Golden Resume Schema.
    Requires user to manually download the 'Profile.csv' archive.
    """
    logger.info("Initializing LinkedIn CSV Migration...")
    if not os.path.exists(csv_path):
        logger.error(
            f"Could not find LinkedIn archive at {csv_path}. "
            "Please download it from LinkedIn settings."
        )
        return
    ...
    try:
        ...
        first_row = df.iloc[0]
        first_n = first_row.get("First Name", "")
        last_n = first_row.get("Last Name", "")
        ...
        resume["basics"]["name"] = (
            name if name else resume["basics"].get("name", "Name")
        )
        resume["basics"]["label"] = (
            headline if headline else resume["basics"].get("label", "")
        )
        resume["basics"]["summary"] = (
            summary if summary else resume["basics"].get("summary", "")
        )
        ...
        logger.info(
            f"Successfully migrated LinkedIn standard profile fields to {output_file}."
        )
...
def push_to_linkedin(input_file: str):
    """
    Generates CSV exports modeled after LinkedIn's native data export formats
...
def main():
    parser = argparse.ArgumentParser(
        description="Bidirectional sync for LinkedIn and Golden Resume."
    )
    parser.add_argument(
        "--direction",
        type=str,
